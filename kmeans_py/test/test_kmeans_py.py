...
def test_kmeans_report():
    ...
    assert model.cluster_summary != None

    # check that summary tables are of type pandas DataFrame
    assert type(model.cluster_summary) is pandas.core.frame.DataFrame
    assert type(model.assignment_summary) is pandas.core.frame.DataFrame
